...
// (csrc/reference/matmul_adds.cu): a 2-CTA-cluster, 2-consumer-warpgroup
// GEMM with TMEM accumulation, followed by an elementwise add of a
// residual tensor C in the epilogue before TMA-storing D.
//
// Tile/pipeline layout follows the proven Blackwell cluster GEMM:
//   Mb=256, Nb=256, Kb=64, 4-stage load pipe, 8-stage epilogue.
// Pages 0..5 hold A/B with D reusing page 0 (same as the framework's
// standalone Gemm). Page 6 is used as a streaming scratch for C:
// one 128x32 residual slice per warpgroup is TMA-loaded from C_gl
// into this page just in time for each of the 8 epilogue stages.
template <typename Config, typename Globals, int SRC_A, int SRC_B, int SRC_C, int DST_D>
struct MatmulAdd {
    ...
    static constexpr int NUM_USED_PAGES = 7; // 4A 2B (D reuses A0) + 1 C scratch
    ...
    using c_st_t = kittens::st_bf<Mb/2, Nb/EPI_PIPE_DEPTH>;   // 128x32 (per-stage residual slice)
    ...
    __device__ static inline kittens::semaphore &c_arrived       (state_t<Config> &s, int cid)   { return s.semaphores()[2*LOAD_PIPE_DEPTH+1+cid]; }
    ...
    __device__ static inline c_st_t &c_st(state_t<Config> &s, int cid)            { return s.pages[s.lid_to_pid(C_LID)].template as<c_st_t>(cid*sizeof(c_st_t)); }
    ...
    struct controller {
        __device__ __forceinline__ static int lid_release_order(const Globals &g, state_t<Config> &s, int query) {
            ...
            // Release order mirrors Gemm's: unused B and A pages flushed quickly; A0 (D) last.
            // C scratch page (lid 6) released right after its last use in the epilogue.
            const int num_iters = g.template gls<SRC_A>().cols() / Kb;
            switch (num_iters % LOAD_PIPE_DEPTH) {
                case 0: case 1: { constexpr int order[] = {2, 1, 3, 5, 4, 6, 0}; return order[query]; }
                case 2:         { constexpr int order[] = {3, 5, 4, 2, 1, 6, 0}; return order[query]; }
                case 3:         { constexpr int order[] = {5, 4, 2, 1, 3, 6, 0}; return order[query]; }
            }
            return 0;
        }
        __device__ __forceinline__ static int init_semaphores(const Globals &g, state_t<Config> &s) {
            ...
            if (lane_id < LOAD_PIPE_DEPTH) {
                ...
                kittens::init_semaphore(c_arrived(s, lane_id - LOAD_PIPE_DEPTH - 1), 1);
            }
    ...
    struct loader {
        __device__ __forceinline__ static void run(const Globals &g, state_t<Config> &s) {
            ...
            auto &c_gl = g.template gls<SRC_C>();
            ...
            if (kittens::warp::elect_leader()) {
                ...
                // Acquire the C page (used only by the consumer's TMA loads in the epilogue).
                s.page_wait(s.lid_to_pid(C_LID));
                // Release fully unused pages (none by default when NUM_USED_PAGES == NUM_PAGES).
                #pragma unroll
    ...
    struct consumer {
        ...
        __device__ __forceinline__ static void run(const Globals &g, state_t<Config> &s) {
            ...
            #pragma unroll
            for (int i = 0; i < EPI_PIPE_DEPTH; i++) {
                // Stream C's i-th column slice into SMEM (one elected leader per warpgroup,
                // one slot per warpgroup in page C_LID). Each warpgroup drives its own slice.
                if (kittens::warpgroup::elect_leader()) {
                    kittens::tma::expect_bytes(c_arrived(s, cid), sizeof(c_st_t));
                    kittens::tma::load_async(
                        c_st(s, cid), c_gl,
                        {(2*tile_row+cta_rank)*NUM_CONSUMERS+cid, EPI_PIPE_DEPTH*tile_col+i},
                        c_arrived(s, cid));
                }
                kittens::wait(c_arrived(s, cid), i&0b1);

                kittens::rt_bf<Mb/8, Nb/EPI_PIPE_DEPTH> c_reg;
                kittens::warpgroup::load(c_reg, c_st(s, cid));
                kittens::warpgroup::add(d_reg[i], d_reg[i], c_reg);
            ...
            if (consumer_group::elect_leader()) {
                s.page_finish(s.lid_to_pid(A_LIDS[0])); // D's staging page
                s.page_finish(s.lid_to_pid(C_LID));    // C's streaming scratch
