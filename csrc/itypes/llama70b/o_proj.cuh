...
// (the O_ProjResidual code path). Every device reads its local shard of A
// and B, computes the matmul, and scatter-adds into its own slice of the
// PGL output D via `tma::store_add_async(d_pgl.gls[dev_idx], ...)`. The
// "residual" in the reference is whatever D already held before this op
// (e.g. the pre-attention hidden state); our kernel accumulates on top,
// so callers should initialize D to the residual value beforehand.
//
// No cross-device traffic: each device writes only to `gls[dev_idx]`. The
// output being declared as a PGL is the *shape* the reference expects
// (hidden_states lives in a pgl across devices), but the op touches only
// the local slice — cross-device fan-out is done elsewhere (attention
// output scatter, DownProj reduce-scatter, explicit barriers).
//
// Inputs SRC_A, SRC_B are per-device regular gls. DST_D must be declared
// as a pgl on the host side. No residual input tensor — the residual is
// the pre-existing value at the PGL slice.
    ...
    struct consumer {
        ...
        __device__ __forceinline__ static void run(const Globals &g, state_t<Config> &s) {
            ...
            for (int i = 0; i < EPI_PIPE_DEPTH; i++) {
                ...
                kittens::warpgroup::sync(cid + 1);
                // Local in-place store-add: each device accumulates only into its own slice.
                if (kittens::warpgroup::elect_leader()) {
                    kittens::tma::store_add_async<kittens::dim::ROW, kittens::cache_policy::EVICT_FIRST>(
                        d_pgl.gls[g.dev_idx], d_st(s, cid, i%NUM_D_TILES),
