...
// the MLP up-projection then multiplies by the previously-materialized
// gate_silu output in registers before storing back. Structurally
// identical to MatmulAdd except the epilogue performs ::mul instead of
// ::add on the streamed C tile.
    ...
    struct consumer {
        ...
        __device__ __forceinline__ static void run(const Globals &g, state_t<Config> &s) {
            ...
            for (int i = 0; i < EPI_PIPE_DEPTH; i++) {
                ...
                kittens::warpgroup::mul(d_reg[i], d_reg[i], c_reg);
