...
// with TMEM accumulation. In the consumer's epilogue, each per-stage bf16
// register tile is transformed in place by SiLU before the TMA store.
...
struct GateSilu {
    ...
    static constexpr int NUM_USED_PAGES = 6; // 4A 2B (D reuses A0)
    ...
    struct consumer {
        ...
        __device__ __forceinline__ static void run(const Globals &g, state_t<Config> &s) {
            ...
            #pragma unroll
            for (int i = 0; i < EPI_PIPE_DEPTH; i++) {
                // silu(x) = x / (1 + exp(-x)). Compute denominator in a scratch tile.
                kittens::rt_bf<Mb/8, Nb/EPI_PIPE_DEPTH> denom;
                kittens::warpgroup::mul(denom, d_reg[i], kittens::bf16(-1.0f));
                kittens::warpgroup::exp(denom, denom);
                kittens::warpgroup::add(denom, denom, kittens::bf16(1.0f));
                kittens::warpgroup::div(d_reg[i], d_reg[i], denom);
