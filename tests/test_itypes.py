...
def collect_test_cases():
    ...
    for cls in megakittens.schema.itype.IType.__subclasses__():
        ...
        if "make_args" not in cls.__dict__:
            raise RuntimeError(f"{cls.__name__} has test_shapes but no make_args")
...
try:
    ...
    def test_itype(itype, shape):
        check(itype.test_fn, itype.make_args(shape), atol=itype.test_atol, rtol=itype.test_rtol)
except ImportError:
    pass
...
if __name__ == "__main__":
    for itype, shape in collect_test_cases():
        max_diff, mean_diff = check(itype.test_fn, itype.make_args(shape), atol=itype.test_atol, rtol=itype.test_rtol)
