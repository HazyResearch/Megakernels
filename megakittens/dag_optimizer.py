    ...
    for node in dag.nodes:
        if node.is_output:
            for edge_idx, ((in_node, in_slot), in_tensor) in enumerate(zip(node.in_nodes, node.in_tensors)):
                if in_tensor.shape != in_node.out_tensors[in_slot].shape:
                    raise RuntimeError(
                        "[MegaKittens] Output node directly consumes a reshaped view;"
                        " returning reshaped views is not supported"
                    )
