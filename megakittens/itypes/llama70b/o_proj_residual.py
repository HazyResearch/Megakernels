...
def _resolve_o_proj_residual(args, kwargs):
    ...
    return OProjResidual(m=m, n=n, k=k), [1]


class OProjResidual(IType):
    ...
    def validate(
        ...
        if a_shape[-2] != M:
            raise RuntimeError(
                f"[MegaKittens] OProjResidual M mismatch: hidden M={M} vs attn_out M={a_shape[-2]}"
            )
        if w_shape[-2] != N:
            raise RuntimeError(
                f"[MegaKittens] OProjResidual N mismatch: hidden N={N} vs o_weights N={w_shape[-2]}"
            )
        if w_shape[-1] != K:
            raise RuntimeError(
        ...
        if N % self.Nb != 0:
            raise RuntimeError(
                f"[MegaKittens] OProjResidual requires N divisible by {self.Nb}, got N={N}"
            )
        if K % self.Kb != 0:
            raise RuntimeError(
                f"[MegaKittens] OProjResidual requires K divisible by {self.Kb}, got K={K}"
            )

        if self.m and M != self.m:
            raise RuntimeError(
                f"[MegaKittens] OProjResidual expected M={self.m}, got {M}"
            )
        if self.n and N != self.n:
            raise RuntimeError(
                f"[MegaKittens] OProjResidual expected N={self.n}, got {N}"
            )
        if self.k and K != self.k:
            raise RuntimeError(
                f"[MegaKittens] OProjResidual expected K={self.k}, got {K}"
            )
