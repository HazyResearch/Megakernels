...
def _use_mixed_tail(m: int, n: int, nb: int) -> bool:
    return m == 1024 and n == 8192 and nb == MIXED_MAIN_NB


def _resolve_nb(m: int, n: int, nb: int = 0) -> int:
    nb = int(nb)
    if nb == 0:
        nb = MIXED_MAIN_NB if m == 1024 and n == 8192 else DEFAULT_NB
    if nb <= 0 or nb > 256 or nb % 32 != 0:
        raise RuntimeError(f"[MegaKittens] OProjResidual70b expected nb in 32..256 step 32, got {nb}")
    if nb % COLS_PER_CHUNK != 0:
        raise RuntimeError(f"[MegaKittens] OProjResidual70b requires nb divisible by {COLS_PER_CHUNK}, got {nb}")
    if n and n % nb != 0 and not _use_mixed_tail(m, n, nb):
        raise RuntimeError(f"[MegaKittens] OProjResidual70b requires N divisible by nb, got N={n}, nb={nb}")
    return nb
...
class OProjResidual70b(IType):
    ...
    bench_cases = [
        ...
        ((512, 8192, 28672), (512, 8192, 28672)),
        ((1024, 8192, 28672), (1024, 8192, 28672)),
        ((2048, 8192, 28672), (2048, 8192, 28672)),
    ]
    ...
    def __init__(self, m: int = 0, n: int = 0, k: int = 0, nb: int = 0, mixed_tail=None, allow_partial_n: bool = False):
        self.m = m
        self.n = n
        self.k = k
    ...
    def inputs(self) -> list[TensorSpec]:
        return [
            TensorSpec(dtype=DType.bf16, granularity=(self.M_INST, self.COLS_PER_CHUNK),
    ...
    def validate(
        ...
        if N % self.COLS_PER_CHUNK != 0:
            raise RuntimeError(
                f"[MegaKittens] OProjResidual70b requires N divisible by {self.COLS_PER_CHUNK}, got N={N}"
            )
