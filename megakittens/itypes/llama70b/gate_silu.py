...
@torch.library.custom_op("megakittens::gate_silu", mutates_args=())
def gate_silu_op(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    return F.silu(a @ b)


@gate_silu_op.register_fake
def _gate_silu_fake(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    return torch.empty(a.shape[0], b.shape[1], dtype=a.dtype, device=a.device)


class GateSilu(IType):
    TILE_M = 512
    TILE_N = 256
    TILE_K = 64
    SUPERGROUP_SIZE = 8
    ...
    def bench_flops(self, case: tuple) -> float:
        M, N, K = case
        return 2.0 * M * N * K  # SiLU is negligible vs matmul
    ...
    def validate(self, src_metas, dst_metas, src_ranges, dst_ranges) -> None:
        ...
        if A_shape[1] != B_shape[0]:
            raise RuntimeError(
                f"[MegaKittens] GateSilu K-dim mismatch: A has K={A_shape[1]}, B has K={B_shape[0]}"
            )
        if A_shape[1] % self.TILE_K != 0:
            raise RuntimeError(
                f"[MegaKittens] GateSilu requires K ({A_shape[1]}) to be a multiple of {self.TILE_K}"
            )
        if D_shape != (A_shape[0], B_shape[1]):
            raise RuntimeError(
                f"[MegaKittens] GateSilu output shape mismatch: expected ({A_shape[0]}, {B_shape[1]}), got {D_shape}"
            )
