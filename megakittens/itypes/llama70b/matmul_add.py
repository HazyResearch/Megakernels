...
@torch.library.custom_op("megakittens::matmul_add", mutates_args=())
def matmul_add_op(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
    return c + a @ b


@matmul_add_op.register_fake
def _matmul_add_fake(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
    return torch.empty_like(c)


class MatmulAdd(IType):
    ...
    def validate(self, src_metas, dst_metas, src_ranges, dst_ranges) -> None:
        ...
        if A_shape[1] != B_shape[0]:
            raise RuntimeError(
                f"[MegaKittens] MatmulAdd K-dim mismatch: A has K={A_shape[1]}, B has K={B_shape[0]}"
            )
        if A_shape[1] % self.TILE_K != 0:
            raise RuntimeError(
                f"[MegaKittens] MatmulAdd requires K ({A_shape[1]}) to be a multiple of {self.TILE_K}"
            )
        if D_shape != (A_shape[0], B_shape[1]):
            raise RuntimeError(
                f"[MegaKittens] MatmulAdd output shape mismatch: expected ({A_shape[0]}, {B_shape[1]}), got {D_shape}"
            )
        if C_shape != D_shape:
            raise RuntimeError(
                f"[MegaKittens] MatmulAdd residual shape must match output: residual={C_shape}, output={D_shape}"
            )
